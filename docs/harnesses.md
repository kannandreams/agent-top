---
description: "What agent-top reads from Claude Code, Codex, Gemini CLI, OpenCode and Kodelet, how it attributes a process to its transcript, and what it does when a harness's format drifts."
---
...
- **harness registry (exact)**: Claude Code writes a per-pid file that names its session, and Kodelet keeps runner and turn records that name the process serving each conversation.
