...
- Cache efficiency (shipped v0.10.0): the share of the prompt served from cache, in the detail pane and a `CACHE` column in the report.
- Context by source (shipped): what each tool's results added to the prompt and what re-reading them since has cost, from prompt growth between responses; the rows sum to the prompt-side cost. OpenCode and a `report --by tool` are follow-ups.
- `agent-top report` (shipped v0.9.0): cross-harness cost and token totals from the transcripts on disk, grouped by harness, model, project or day.
