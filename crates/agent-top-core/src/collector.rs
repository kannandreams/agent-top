    ...
    pub fn collect(&mut self) -> Snapshot {
        ...
        for root in roots {
            ...
            for (i, (path, mut summary)) in sessions.into_iter().enumerate() {
                ...
                agents.push(Agent {
                    ...
                    folds_child_usage: summary.folds_child_usage,
                    tool_calls: summary.tool_calls,
                    tool_calls_lower_bound: summary.tool_calls_lower_bound,
                    ...
                    context: summary.context.sources(),
        ...
        for (p, harness) in stopped {
            ...
            agents.push(Agent {
                ...
                price_source: s.price_source.or_else(|| s.model.as_deref().and_then(|m| crate::pricing::table().source_for(m))),
